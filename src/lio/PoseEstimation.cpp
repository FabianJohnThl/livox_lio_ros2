...
static inline double get_time_sec(const builtin_interfaces::msg::Time& time) {
...
std::shared_ptr<rclcpp::Publisher<nav_msgs::msg::Path>> pubLaserOdometryPath;
...
std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::NavSatFix>> pubGps;

bool newfullCloud = false;

Eigen::Matrix4d transformAftMapped{Eigen::Matrix4d::Identity()};

std::mutex mutexLidarQueue;
std::mutex mutexIMUQueue;
std::queue<sensor_msgs::msg::PointCloud2::ConstSharedPtr> lidarMsgQueue;
std::queue<sensor_msgs::msg::Imu::ConstSharedPtr> imuMsgQueue;
 ...
 */
void pubOdometry(const Eigen::Matrix4d& newPose, double& time_full_cloud) {
    ...
    laserOdometry.header.stamp            = rclcpp::Time(static_cast<int64_t>(time_full_cloud * 1e9));
    ...
    laserOdoPath.header.frame_id = "lidar_init";
    pubLaserOdometryPath->publish(laserOdoPath);

    laserOdometryTrans.header.frame_id         = "lidar_init";
    ...
    gps.header.stamp        = rclcpp::Time(static_cast<int64_t>(time_full_cloud * 1e9));
    gps.header.frame_id     = "lidar_init";
    gps.latitude            = newPosition.x();
    gps.longitude           = newPosition.y();
    gps.altitude            = newPosition.z();
    gps.position_covariance = {Rcurr(0, 0), Rcurr(1, 0), Rcurr(2, 0), Rcurr(0, 1), Rcurr(1, 1),
                               Rcurr(2, 1), Rcurr(0, 2), Rcurr(1, 2), Rcurr(2, 2)};
    pubGps->publish(gps);
}

void fullCallBack(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg) {
    // push lidar msg to queue
    std::unique_lock<std::mutex> lock(mutexLidarQueue);
    lidarMsgQueue.push(msg);
}

void imu_callback(const sensor_msgs::msg::Imu::ConstSharedPtr& imu_msg) {
    // push IMU msg to queue
    std::unique_lock<std::mutex> lock(mutexIMUQueue);
    imuMsgQueue.push(imu_msg);
}
 ...
 */
bool fetchImuMsgs(
    double startTime, double endTime, std::vector<sensor_msgs::msg::Imu::ConstSharedPtr>& vimuMsg) {
    std::unique_lock<std::mutex> lock(mutexIMUQueue);
    ...
    while (true) {
        if (imuMsgQueue.empty())
            break;
        if (get_time_sec(imuMsgQueue.back()->header.stamp) < endTime
            || get_time_sec(imuMsgQueue.front()->header.stamp) >= endTime)
            break;

        sensor_msgs::msg::Imu::ConstSharedPtr& tmpimumsg = imuMsgQueue.front();

        double time = get_time_sec(tmpimumsg->header.stamp);
        ...
        if (time <= endTime && time > startTime) {
            ...
            if (time <= startTime) {
                ...
                sensor_msgs::msg::Imu::SharedPtr theLastIMU(new sensor_msgs::msg::Imu);
                theLastIMU->linear_acceleration.x =
                    w1 * vimuMsg.back()->linear_acceleration.x + w2 * tmpimumsg->linear_acceleration.x;
                theLastIMU->linear_acceleration.y =
                    w1 * vimuMsg.back()->linear_acceleration.y + w2 * tmpimumsg->linear_acceleration.y;
                theLastIMU->linear_acceleration.z =
                    w1 * vimuMsg.back()->linear_acceleration.z + w2 * tmpimumsg->linear_acceleration.z;
                theLastIMU->angular_velocity.x =
                    w1 * vimuMsg.back()->angular_velocity.x + w2 * tmpimumsg->angular_velocity.x;
                theLastIMU->angular_velocity.y =
                    w1 * vimuMsg.back()->angular_velocity.y + w2 * tmpimumsg->angular_velocity.y;
                theLastIMU->angular_velocity.z =
                    w1 * vimuMsg.back()->angular_velocity.z + w2 * tmpimumsg->angular_velocity.z;
...
void RemoveLidarDistortion(
    pcl::PointCloud<PointType>::Ptr& cloud, const Eigen::Matrix3d& dRlc, const Eigen::Vector3d& dtlc) {
    auto PointsNum = cloud->points.size();
    for (int i = 0; i < PointsNum; i++) {
        ...
        startP = delta_qlc * Eigen::Vector3d(cloud->points[i].x, cloud->points[i].y, cloud->points[i].z)
               + delta_Plc;
...
bool TryMAPInitialization() {
    ...
    auto v_size = lidarFrameList->size();
    prior_v.reserve(v_size);

    for (int i = 0; i < v_size; i++) {
        prior_v.emplace_back(Eigen::Vector3d::Zero());
    }
    ...
    problem.AddResidualBlock(
        Cost_Initialization_Prior_R::Create(prior_r, sqrt_information_r), nullptr, para_r);

    problem.AddResidualBlock(
        Cost_Initialization_Prior_bv::Create(prior_ba, sqrt_information_ba), nullptr, para_ba);
    problem.AddResidualBlock(
    ...
    if (ba_vec.norm() > 0.5 || bg_vec.norm() > 0.5) {
        RCLCPP_WARN(rclcpp::get_logger("pose_estimation"), "Too Large Biases! Initialization Failed!");
        return false;
    }
    ...
    for (int i = 0; i < v_size; i++) {
        ...
        if ((bv_vec - prior_v[i]).norm() > 2.0) {
            RCLCPP_WARN(rclcpp::get_logger("pose_estimation"), "Too Large Velocity! Initialization Failed!");
    ...
    WINDOW_SIZE = Estimator::SLIDE_WINDOW_SIZE;
    while (lidarFrameList->size() > WINDOW_SIZE) {
...
void process() {
    ...
    while (rclcpp::ok()) {
        ...
        std::unique_lock<std::mutex> lock_lidar(mutexLidarQueue);
        if (!lidarMsgQueue.empty()) {
            // get new lidar msg
            time_curr_lidar = get_time_sec(lidarMsgQueue.front()->header.stamp);
            pcl::fromROSMsg(*lidarMsgQueue.front(), *laserCloudFullRes);
            lidarMsgQueue.pop();
            newfullCloud = true;
        }
        lock_lidar.unlock();
        ...
        if (newfullCloud) {
            ...
            delta_Rb =
                transformAftMapped.topLeftCorner(3, 3).transpose() * lidar_list->front().Q.toRotationMatrix();
            delta_tb = transformAftMapped.topLeftCorner(3, 3).transpose()
            ...
            Eigen::Vector3d Pwlpre =
                transformAftMapped.topLeftCorner(3, 3) * exPbl + transformAftMapped.topRightCorner(3, 1);
            ...
            auto laserCloudFullResNum = lidar_list->front().laserCloud->points.size();
            ...
            for (int i = 0; i < laserCloudFullResNum; i++) {
                ...
                MAP_MANAGER::pointAssociateToMap(
                    &lidar_list->front().laserCloud->points[i], &temp_point, transformTobeMapped);
            ...
            laserCloudMsg.header.stamp =
                rclcpp::Time(static_cast<int64_t>(lidar_list->front().timeStamp * 1e9));
            ...
            if (IMU_Mode > 1 && !LidarIMUInited) {
                ...
                if (pushCount == 0) {
                    ...
                    if (lidarFrameList->size() > WINDOW_SIZE)
                        lidarFrameList->pop_front();
                } else {
                ...
                if (pushCount >= 3) {
                    ...
                    if (lidarFrameList->size() > 1) {
                        ...
                        iterRight->imuIntegrator.PreIntegration(
                            iterLeft->timeStamp, iterLeft->bg, iterLeft->ba);
                    }

                    if (lidarFrameList->size() == int(WINDOW_SIZE / 1.5)) {
                        startTime = lidarFrameList->back().timeStamp;
                    }

                    if (!LidarIMUInited && lidarFrameList->size() == WINDOW_SIZE
                        && lidarFrameList->front().timeStamp >= startTime) {
                        std::cout << "**************Start MAP Initialization!!!******************"
                                  << std::endl;
                        ...
                        std::cout << "**************Finish MAP Initialization!!!******************"
                                  << std::endl;
                    }
                }
            }
...
int main(int argc, char** argv) {
    ...
    auto node   = std::make_shared<rclcpp::Node>("PoseEstimation", option);
    ...
    node->get_parameter("IMU_Mode", IMU_Mode);
    node->get_parameter("Extrinsic_Tlb", vecTlb);

    RCLCPP_INFO(node->get_logger(), "[filter_corner][%.3f]", filter_parameter_corner);
    RCLCPP_INFO(node->get_logger(), "[filter_surf][%.3f]", filter_parameter_surf);

    // set extrinsic matrix between lidar & IMU
    Eigen::Matrix3d r;
    Eigen::Vector3d t;
    r << vecTlb[0], vecTlb[1], vecTlb[2], vecTlb[4], vecTlb[5], vecTlb[6], vecTlb[8], vecTlb[9], vecTlb[10];
    ...
    rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr full_cloud_subscription;
    rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr imu_subscription;

    full_cloud_subscription =
        node->create_subscription<sensor_msgs::msg::PointCloud2>("/lio_livox/full_cloud", 10, fullCallBack);

    if (IMU_Mode > 0)
        imu_subscription = node->create_subscription<sensor_msgs::msg::Imu>("/livox/imu", 2000, imu_callback);
    if (IMU_Mode < 2)
        WINDOW_SIZE = 1;
    else
        WINDOW_SIZE = 20;
    ...
    pubFullLaserCloud =
        node->create_publisher<sensor_msgs::msg::PointCloud2>("/lio_livox/full_cloud_mapped", 10);
    pubLaserOdometry     = node->create_publisher<nav_msgs::msg::Odometry>("/lio_livox/odometry_mapped", 5);
    pubLaserOdometryPath = node->create_publisher<nav_msgs::msg::Path>("/lio_livox/odometry_path_mapped", 5);
    pubGps               = node->create_publisher<sensor_msgs::msg::NavSatFix>("/lio_livox/lidar", 1000);

    tfBroadcaster     = std::make_shared<tf2_ros::TransformBroadcaster>(node);
    laserCloudFullRes = std::make_shared<pcl::PointCloud<PointType>>();
    lidarFrameList    = std::make_shared<std::list<Estimator::LidarFrame>>();
